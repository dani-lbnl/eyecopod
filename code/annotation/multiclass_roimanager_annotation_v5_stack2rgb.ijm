...
macro "Annotate Image [F5]" {
	...
	imagename = filename;
	win = getTitle();
	setTool("wand"); run("Wand Tool...", "tolerance=5 mode=4-connected");
	setTool("freeline");
	run("Line Width...", "line=" + rootwidth);
	...
	run("Tile");
	waitForUser("Ready to create mask? Fix it manually if you need to before hitting okay.");
	selectWindow("Mask");
...
function ui_initialization(){
 ...
 print("Start annotation by selecting the roots!") 
